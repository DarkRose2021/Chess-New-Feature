...
$(function () {
	...
	uidFromCookie = Cookie.get("chess")
		? JSON.parse(Cookie.get("chess")).uid
		: null;
	...
	if (!uidFromURL) {
		...
		if (!uidFromCookie) {
			...
			if (
				uidFromCookie.substr(0, uidFromCookie.length - 1) !==
					uidFromURL.substr(0, uidFromURL.length - 1) &&
				confirm(
					"Do you want to discard your current game and join this new game? "
				)
			) {
				uid = uidFromURL;
			} else {
	...
	$("#info .opponent-link")
		.html("http://diovo.com/chess/?game=" + opponentUid)
		.attr("href", "/chess/?game=" + opponentUid);
	...
	$(".column div")
		.filter(function () {
			...
			return (
				$(this)
					.attr("class")
					.indexOf(user.color ? "WHITE" : "BLACK") !== -1
			);
		})
...
function refreshFromServer() {
	$.ajax({
		...
		error: function () {
			$("#info .error").text(
				"Could not connect to the server. You will not be able to play a multiplayer game"
			);
			drawBoard(board);
		},
	});
